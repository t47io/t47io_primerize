...
const drawIllustration2D = (props) => {
  const { sequence, offset, startPos, endPos } = props;

  const startIdx = startPos + offset - 1;
  const endIdx = endPos + offset - 1;
  const fragments = [];
  ...
  const labels = [`${1 - offset}`, `${startPos}`, `${endPos}`, `${sequence.length - offset}`];
...
const drawIllustration3D = (props) => {
  const { sequence, structures, offset, startPos, endPos } = props;
